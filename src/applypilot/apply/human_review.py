...
def _inject_banner(port: int, job: dict, server_port: int = 7373) -> bool:
    ...
    company = (job.get("site") or job.get("company") or "").replace("`", "'")
...
(async () => {{
  try {{
    ...
    if (pages.length > 0) {{
      await pages[0].evaluate(bannerJs);
    }}
    await b.close();
    process.exit(0);
  }} catch(e) {{
...
(function() {{
  ...
  function _injectBanner() {{
    ...
    info.innerHTML = '<strong>&#9872; ApplyPilot HITL</strong>'
      ...
      + '<br><span style="font-size:12px;opacity:0.9">{instructions}</span>';

    var btn = document.createElement('button');
    btn.textContent = 'Done — Resume Agent';
    btn.style.cssText = [
    ...
    btn.onclick = function() {{
      ...
      // Signal the background watcher via a window variable (not subject to CSP).
      // The Node.js watcher polls this via CDP and calls the server from outside
      // the page context, bypassing any site Content-Security-Policy.
      window.__ap_hitl_done = '{hash_}';
      // Fast path: also try a direct fetch (works on sites without restrictive CSP).
      // Errors are silenced — the watcher will complete the handoff regardless.
      fetch('http://localhost:{server_port}/api/done/{hash_}', {{method:'POST'}})
        .then(function(r) {{
          if (r.ok) {{
            btn.textContent = 'Agent taking over...';
      ...
      try {{
        ...
        for (const page of pages) {{
          ...
          if (val === '{hash_}') {{
            ...
            // Update banner button to confirmed state
            await page.evaluate(`(function() {{
              var btn = document.querySelector('#__ap_banner_root button');
              if (btn) {{
                btn.textContent = 'Agent taking over...';
                btn.style.background = '#22c55e';
                btn.style.color = '#000';
...
class _Handler(BaseHTTPRequestHandler):
    ...
    def _handle_done(self, h: str) -> None:
        """User clicked Done — spawn agent thread."""
        with _sessions_lock:
            session = _sessions.get(h)
